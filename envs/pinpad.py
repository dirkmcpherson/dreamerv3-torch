...
class PinPad(gym.Env):
  ...
  def observation_space(self):
    ...
    return gym.spaces.Dict({
        ...
        'is_first': gym.spaces.Box(low=0, high=1, shape=(), dtype=np.bool),
        'is_last': gym.spaces.Box(low=0, high=1, shape=(), dtype=np.bool),
        'is_terminal': gym.spaces.Box(low=0, high=1, shape=(), dtype=np.bool),
    })

  def step(self, action):
    # print(f"pinpad action {action}")
    if self.done:
      return self.reset()
    if self.countdown:
      self.countdown -= 1
    ...
    obs = self._obs(reward=reward, is_last=self.done)
